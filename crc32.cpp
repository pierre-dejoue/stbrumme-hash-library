...
{
  /// look-up table
  static const std::uint32_t crc32Lookup[8][256] =
  {
    // generated by:
...
void CRC32::add(const void* data, std::size_t numBytes)
{
  std::uint32_t* current = (std::uint32_t*) data;
  std::uint32_t crc = ~m_hash;
...
std::string CRC32::operator()(const void* data, std::size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
